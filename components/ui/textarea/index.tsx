...
const textareaInputStyle = tva({
  base: "p-2 web:outline-0 web:outline-none color-typography-900 !align-text-top placeholder:text-typography-500 web:cursor-text web:data-[disabled=true]:cursor-not-allowed",
  parentVariants: {
    size: {
      sm: "text-sm",
      md: "text-base",
  ...
  return (
    <UITextarea
      ...
      className={textareaStyle({
        variant,
        class: `bg-white rounded-lg ${className}`,
      })}
      context={{ size }}
    />
  );
  ...
  return (
    <UITextarea.Input
      ...
      className={textareaInputStyle({
        ...
        class: `${className} bg-white rounded-lg`,
      })}
    />
  );
});
